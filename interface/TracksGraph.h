  ...
  void findSubComponents(std::vector<Node>& graph, std::vector<int>& subComponent, float threshold) {
    if (!alreadyVisited_) {
      ...
      for (; neighbourIt != neighboursId_.cend() && weightIt != weights_.cend(); ++neighbourIt, ++weightIt) {
          ...
          const Node& neighbourNode = graph[neighbour];
          double weight = *weightIt;
	  std::cout<<" index_ "<<index_<<" neighbour "<<neighbour<<" weights "<<weight<<" zpositon "<<this->getZPosition()<<" neighbour Z "<<neighbourNode.getZPosition()<<std::endl;
          if (weight >= threshold){
...
public:
  ...
  void setEdgeWeight(unsigned int nodeIndexI, unsigned int nodeIndexJ, float weight) {
	...
	std::cout<<" SetEdge Weight "<<nodeIndexJ <<" nodeIndexI "<<nodeIndexI<<" wiegth "<<weight<<std::endl;
  }
 // DFS algorithm 
